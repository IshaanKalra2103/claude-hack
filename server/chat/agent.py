...
class ShellMateChatAgent:
    ...
    async def _try_agents_sdk(self, *, context: dict[str, Any], user_text: str) -> str | None:
        ...
        scraper_agent = Agent(
            ...
            instructions=(
                "You are ShellMate's background research agent. Use scraper_search for deeper lookups, summarize only the useful bits, and return them to the main chat agent."
            ),
            model=settings.openai_scraper_model,
            tools=[scraper_search],
        )
        chat_agent = Agent(
            name="ShellMate",
            instructions=(
